...
def inference(
    ...
    diffusion.load_state_dict(torch.load(f'{checkpoint_path}/diffusion_weights.pt', weights_only=True))
    ...
    with torch.no_grad():
        for i, index in enumerate(tqdm(np.random.randint(0,test_dataset.__len__(),samples), desc=f'Sample {n}/{samples}')):
            ...
            for t in reversed((range(1,time_steps))):
                ...
                z = (1/(torch.sqrt(1-noise_sched.beta[t].to(device))))*z - (temp.to(device)*diffusion(z,t))

                # Save selected intermediate samples
                if t[0] in times:
                    images.append(z.squeeze(1).unsqueeze(0).cpu())
                
                # compute x_unknown = x_{t-1} according to DDPM paper
            ...
            z = (1/(torch.sqrt(1-noise_sched.beta[0])))*z - (temp*diffusion(z,[0]))
...
if __name__ == '__main__':
    ...
    output_path = '../diffusion_v1/output_total'
